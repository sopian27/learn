...
**Note on the default domain:** for the default `software-engineering` domain, the existing SWE-specific loop described in root `CLAUDE.md` ("Learning Loop") and `ai-los/LEARNING_MODE.md` ("Lesson Structure") — Theory → Visualization → Simple Example → Real-world Example → Coding → Exercises → Code Review → Common Mistakes → Interview Questions/Quiz → Summary — IS that domain's concrete instantiation of this Universal Learning Loop, not a separate, competing process. `CLAUDE.md`'s "Coding" step is this loop's Applied Practice step, "Code Review" is this loop's Review step, and "Interview Questions"/"Quiz" is this loop's Assessment step. Every other domain instantiates the same ten-step loop with its own domain-appropriate labels, defined in that domain's `DOMAIN.md`.
