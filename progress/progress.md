...
* 2026-09-02: Lesson 3.1 (Memory model JVM: heap vs stack vs metaspace, object lifecycle, eligible GC) selesai, skor 90/100. Reachability-based GC eligibility dipegang solid, diagnosis Insiden B (metaspace/classloader leak dari RSS naik tapi heap metric stabil) tepat tanpa dituntun. Dua catatan turun skor: object yang sama dilihat dari 2 titik kode (Bagian 1 poin 3 & 6) belum disambungkan eksplisit, dan unbounded cache belum ditandai sebagai pola leak (bukan cuma soal GC eligibility teoretis). Lanjut Lesson 3.2 (Garbage Collector) — materi + Exercise sudah ditulis ke vault, menunggu jawaban.
* 2026-09-03: Lesson 3.2 (Garbage Collector: mental model generasional, GC modern G1/ZGC, trade-off throughput vs latency) selesai, skor 82/100. Bagian 2 (diagnosis 3 skenario collector) kuat penuh — pilihan tepat dengan reasoning trade-off non-template, sadar `MaxGCPauseMillis` cuma soft goal. Bagian 3.1 (copying collector Young Gen) tepat, Bagian 3.3 (colored pointer/load barrier) ada self-koreksi bagus atas analogi sendiri. Catatan turun skor: (1) Bagian 1 — data eksperimen ZGC pause 21.339 ms bertentangan langsung dengan Theory (harusnya sub-ms), tapi user tidak menginvestigasi anomali ini meski soal eksplisit minta cek kesesuaian dengan Theory; (2) Bagian 3.2 belum menjawab reframe "Full GC di G1 = fallback/failure mode" vs "Full GC di GC klasik = kejadian rutin"; (3) Bagian 3.3 risiko correctness ZGC baru cover stale read, belum sisi write/use-after-free. Lanjut Lesson 3.3 (Memory leak klasik Spring Boot).
...
* 2026-09-10: Mini Project Module 3 (diagnosis & perbaikan memory leak `SessionAnalyticsService`) selesai dijawab dan direview, skor 70/100 — lebih rendah dari lesson-lesson sebelumnya karena celah eksekusi, bukan celah konsep. Deliverable 1 (pattern recognition Pola 1 Lesson 3.3 + rantai reachability) dan Deliverable 3 (manual eviction dipilih dengan alasan tepat — flush sebagai business event, bukan tebakan waktu seperti TTL — dan berfungsi nyata di `SessionAnalyticsDemoV2.java`) solid, begitu juga Production Discussion (distinction thread-safety `ConcurrentHashMap` vs lifecycle/reachability tidak tertukar). Masalah utama: Deliverable 2 minta heap dump diambil lewat `jcmd GC.heap_dump` lalu dominator tree dibuka di VisualVM/MAT, tapi yang dilampirkan justru **thread dump** (`Thread.print`/`jstack`) — artefak yang salah jenis sama sekali, dan tidak ada file `.hprof` yang pernah dihasilkan. Data "before" di Deliverable 4 juga memakai thread dump yang sama persis, sehingga perbandingan before/after tidak apple-to-apple. Rekomendasi: ulangi Deliverable 2 & 4 dengan command dan tool yang benar sebelum menganggap Module 3 tuntas penuh — detail lengkap ada di vault. Module 3 secara konsep sudah utuh (5/5 lesson + Mini Project direview), tapi evidence Mini Project belum final; bisa lanjut ke Module 4 (Spring Core & IoC Deep Dive) sambil evidence ini menyusul, atau diulang dulu.
* 2026-09-10: Module 4 dimulai. Lesson 4.1 (IoC Container & `ApplicationContext`: bean definition, bean lifecycle) materi + Exercise/Task digenerate ke vault, menunggu jawaban.
* 2026-09-11: Lesson 4.1 selesai dijawab dan direview, skor 88/100. Bagian 1 (mekanisme NPE field injection diakses di constructor) dan Bagian 2 (urutan 6 callback init/destroy) tepat. Bagian 3 jadi bagian terkuat — jawaban soal titik pembuatan AOP proxy (`postProcessAfterInitialization`) disambungkan sendiri ke self-invocation problem (`this.methodLain()` tidak melewati proxy karena `this` menunjuk object asli), padahal itu materi Lesson 4.4 yang belum diajarkan. Interview Q1-Q6 semua tepat. Satu catatan turun skor: exercise minta 2 cara berbeda memperbaiki NPE, cuma dijawab 1 (constructor injection) — cara kedua (`@PostConstruct`, tetap pakai field injection) tidak disebut. Juga ada artefak encoding `â??` di output console (sama seperti review Module 3, murni codepage Windows bukan UTF-8, bukan sinyal bug). Detail lengkap ada di vault. Lanjut ke Lesson 4.2 (Dependency Injection: constructor vs setter vs field injection).
