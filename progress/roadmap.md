...
3. **Freelance & International Career Readiness** — **sekarang punya draft course**: `courses/freelance-readiness/roadmap.md` (dibuat 2026-08-29, menunggu approval). Sisa gap operasional bisnis freelance (Upwork/Toptal profile, proposal, kontrak, pembayaran internasional, pajak, reputasi) setelah sisi kredensial dipecah ke `courses/credential-strategy` dan sisi kemasan/distribusi dipecah ke `courses/career-reinvention`. `ai-los/CAREER_MODE.md` tetap jadi kerangka mentoring komunikasi Bahasa Inggris yang berjalan paralel, bukan pengganti course ini.
...
* **Backend Engineering Fundamentals Rebuild** — `courses/backend-fundamentals/roadmap.md` — Status: In Progress (Module 1 tuntas, skor Mini Project 90/100 — Module 2 Concurrency & Multithreading tuntas sepenuhnya: lesson 2.1-2.5 (84/80/82/91/92) + Mini Project Module 2 skor 93/100 — Module 3 JVM Internals: lesson 3.1 (90/100), 3.2 (82/100), 3.3 Memory leak klasik Spring Boot (85/100), utang Deliverable 2 & 4 Mini Project belum diulang pakai `jcmd GC.heap_dump`+VisualVM — Module 4 Selesai (2026-09-20) — Module 5 REST API Production-Grade: lesson 5.1 (90/100), 5.2 (92/100), 5.3 Validation layer (94/100, revisi dari 84/100), 5.4 OpenAPI/Swagger (90/100) — lanjut Lesson 5.5 Idempotency & ETag) — **resume 2026-08-19 atas permintaan eksplisit user**, override kebijakan cap 2-course-aktif 2026-08-07 (slot Mastering Claude sudah selesai jadi user memilih isi dengan course ini alih-alih menganggap kosong).
...
* **Personal Finance & Financial Independence — dari 0 sampai Punya Rencana FI Lengkap** — `courses/personal-finance/roadmap.md` — Domain: personal-finance — Module 1, Lesson 1.1 — pilot domain non-SWE pertama untuk arsitektur universal AI-LOS. **Diperluas 2026-08-25** (permintaan langsung "Financial Independence 0-expert usia 33"): 3 module baru (Housing & Property, Passive Income & FI Number, Children Costs & Education Fund) + rekalibrasi Retirement Planning & Capstone ke framing FI usia 33/tidak membebani anak — 12→15 module, direnumber bersih karena masih 0% progress. **Diperluas lagi 2026-09-01** (permintaan langsung "Financial Freedom 0-expert": budgeting/saving/dana darurat/emas/investasi/rumah/mobil/modal menikah/passive income/asset allocation/retirement/FI) — cek silang menunjukkan ~90% sudah tercakup, hanya emas/mobil/modal menikah yang gap: 3 module baru (Marriage & Wedding Capital Planning, Vehicle Ownership & Car Buying Decisions, Gold & Precious Metals) — 15→18 module, direnumber bersih lagi (masih 0% progress). Kedua kali diputuskan **extend course existing**, bukan course baru, untuk hindari duplikasi. **Dibuat/diperluas sambil kedua Course Aktif ("Character Development" Module 1/6, "Backend Engineering Fundamentals Rebuild" Module 2/15) masih di bawah 50% — user eksplisit override guard `CLAUDE.md` § Course Creation Discipline saat ditanya konfirmasi 2026-09-01.** Draft menunggu approval, belum ada Lesson yang dimulai.
...
## 2026-09-01 — AI-Native Backend Engineering (permintaan langsung, override guard Course Creation Discipline)

* **AI-Native Backend Engineering — dari 0 sampai Expert** — `courses/ai-native-backend/roadmap.md` — Domain: software-engineering — Status: Draft 2026-09-01, menunggu approval — 11 modul + Enterprise Project: lanskap arsitektur AI-native backend → Spring AI fundamentals (ChatClient, provider abstraction) → Advisor/RAG/MCP dengan Spring AI → Redis untuk AI (RedisTemplate JSON, semantic cache, agent memory) → open table format (Iceberg vs Delta vs Hudi) sebagai lapisan data AI → strategi anti vendor lock-in (gateway vs abstraksi in-process) → Kubernetes untuk AI workload (DRA, Kueue, KServe, KEDA) → Kubeflow & lanskap platform ML (vs Ray/MLflow) → AI observability (OTel GenAI semconv, Langfuse/LangSmith) → keamanan AI/LLM (OWASP Top 10 for LLM Applications) → compliance & governance AI untuk engineer (EU AI Act, NIST AI RMF). Riset fresh 2026-09-01 (Spring AI reference docs, CNCF DRA blog, EU AI Act implementation timeline, OWASP GenAI Security Project, dll — daftar lengkap di "Catatan Scope" `courses/ai-native-backend/roadmap.md`), beberapa topik ditandai eksplisit "masih berubah cepat" (Spring AI 2.0 masih milestone, OTel GenAI semconv masih experimental, timeline EU AI Act sudah bergeser sekali). Perspektif backend/platform engineer (Java/Spring) yang membungkus AI ke sistem production — sengaja tidak overlap dengan `courses/llm`/`courses/agents` (jalur Python, arsitektur model/agent), `courses/kubernetes` (fondasi umum), `courses/observability` (tiga pilar generik), atau `courses/cybersecurity` (OWASP web klasik) — rasional pemisahan lengkap per-course ada di "Catatan Scope". **Dibuat sambil kedua Course Aktif ("Backend Engineering Fundamentals Rebuild" Module 2/15, "Character Development" Module 1/6) masih di bawah 50% — user eksplisit override guard CLAUDE.md § Course Creation Discipline saat ditanya konfirmasi.**

## 2026-08-29 — Freelance & International Career Readiness (via `/recommend`, permintaan langsung, override guard Course Creation Discipline)

* **Freelance & International Career Readiness — dari Profil Kosong sampai Client Internasional Pertama** — `courses/freelance-readiness/roadmap.md` — Domain: software-engineering (tidak perlu domain baru, non-teknis tapi konteks software engineer) — Status: Draft 2026-08-29, menunggu approval — 8 modul + Enterprise Project: landscape platform (Upwork vs Toptal vs direct outreach) → profil Upwork SEO & jalur vetting Toptal (4 tahap) → proposal writing & outreach → pricing strategy (hourly/fixed/value-based) → kontrak & IP protection → pembayaran internasional (Payoneer/Wise) & kewajiban pajak Indonesia → client management lintas zona waktu → reputasi/badge system & pertumbuhan jangka panjang. Riset fresh Agustus 2026 (Upwork Help resmi, DDIY/HoningJS untuk vetting Toptal, xflowpay untuk perbandingan fee Payoneer/Wise, Ortax/DJP untuk kewajiban pajak freelancer Indonesia) — daftar lengkap di "Catatan Scope" `courses/freelance-readiness/roadmap.md`. **Menutup gap Master Goal terakhir yang belum punya course sama sekali** — sisi kredensial sudah `courses/credential-strategy`, sisi kemasan/distribusi sudah `courses/career-reinvention`, course ini murni operasional bisnis freelance. **Dibuat sambil kedua Course Aktif ("Character Development" Module 1/6, "Backend Engineering Fundamentals Rebuild" Module 2/15) masih di bawah 50% — user eksplisit override guard `CLAUDE.md` § Course Creation Discipline saat ditanya konfirmasi.**

# Course Belum Dibuat (gap dari Master Goal)

* Tidak ada — seluruh gap di Gap Analysis di atas sudah punya course (aktif, disetujui, atau draft menunggu approval). Update 2026-08-29: gap terakhir "Freelance & International Career Readiness" ditutup oleh `courses/freelance-readiness/roadmap.md`.
